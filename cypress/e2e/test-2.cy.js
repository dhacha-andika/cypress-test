// Example Cypress script to scrape data from two websites
...
describe('Scrape and Compare Prices', () => {
    ...
    cy.get('.centerColAlign').each(($el) => {
        ...
        const price = parseFloat($el.find('.a-price-whole').text().replace(/[^0-9.]/g, '')) // Convert price to number
        products.push({ name, price })
      })
  
      // Scrape second website
    //   cy.origin('https://www.hellostore.id', () => {
        // Add the commands targeting hellostore.id here
    ...
    //     const price = parseFloat($el.find('.item-price').text().replace(/[^0-9.]/g, '')) // Convert price to number
    //     products.push({ name, price })
    //   })
